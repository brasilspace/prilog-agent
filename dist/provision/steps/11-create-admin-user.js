...
exports.getSynapseAdminToken = getSynapseAdminToken;
exports.stepCreateAdminUser = stepCreateAdminUser;
exports.verifyCreateAdminUser = verifyCreateAdminUser;
const logger_js_1 = require("../../utils/logger.js");
const safe_exec_js_1 = require("../engine/safe-exec.js");
// ─── Container Name ermitteln ─────────────────────────────────────────────────
async function getSynapseContainerName() {
    const candidates = ['prilog-synapse-1', 'synapse'];
    for (const name of candidates) {
        const result = await (0, safe_exec_js_1.safeExec)('docker', ['inspect', '--format={{.State.Running}}', name], { ignoreExitCode: true, timeout: 5_000 });
        if (result.exitCode === 0 && result.stdout.trim().replace(/'/g, '') === 'true')
            return name;
    }
    throw new Error('Synapse Container nicht gefunden');
}
// ─── Admin-User existiert bereits? ───────────────────────────────────────────
async function adminUserExists(cfg) {
    try {
        const result = await (0, safe_exec_js_1.dockerExec)('prilog-postgres-1', [
...
async function getSynapseAdminToken(cfg) {
    ...
    const result = await (0, safe_exec_js_1.safeExec)('curl', ['-sf', '-X', 'POST', 'http://localhost:8008/_matrix/client/v3/login',
        '-H', 'Content-Type: application/json',
        '-d', loginPayload,
        '--max-time', '15'], { timeout: 20_000 });
...
// ─── Step ─────────────────────────────────────────────────────────────────────
async function stepCreateAdminUser(cfg) {
    // ── Idempotenz: User bereits vorhanden? ──────────────────────────
    const exists = await adminUserExists(cfg);
    if (exists) {
        logger_js_1.logger.info(`[Step 7] Admin-User @${cfg.adminUsername} bereits vorhanden — überspringe`);
        return;
    }
    const containerName = await getSynapseContainerName();
    logger_js_1.logger.info(`[Step 7] Erstelle Admin-User @${cfg.adminUsername} via ${containerName}...`);
    // ── register_new_matrix_user ausführen ────────────────────────────
    // Passwort aus Base64 dekodieren
    const adminPasswordDecoded = Buffer.from(cfg.adminPasswordB64, 'base64').toString('utf8');
    try {
        const result = await (0, safe_exec_js_1.dockerExec)(containerName, [
        ...
        if (output.includes('Success') || output.includes('success')) {
            logger_js_1.logger.info(`[Step 7] Admin-User @${cfg.adminUsername} erstellt`);
        }
        else if (output.includes('already exists') || output.includes('already in use')) {
            logger_js_1.logger.info(`[Step 7] Admin-User @${cfg.adminUsername} existiert bereits (OK)`);
        }
        else {
            logger_js_1.logger.info(`[Step 7] register_new_matrix_user Output: ${output}`);
        }
    }
    catch (err) {
    ...
    logger_js_1.logger.info('[Step 7] Hole Synapse Admin-Token...');
    try {
        const token = await getSynapseAdminToken(cfg);
        cfg.synapseAdminToken = token;
        logger_js_1.logger.info('[Step 7] Synapse Admin-Token erhalten');
    }
    catch (err) {
        // Nicht fatal — finalize-Step sendet was vorhanden ist
        logger_js_1.logger.warn(`[Step 7] Synapse Admin-Token konnte nicht geholt werden: ${err.message}`);
    }
}
async function verifyCreateAdminUser(cfg) {
    const exists = await adminUserExists(cfg);
    if (!exists) {
