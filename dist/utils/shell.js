...
async function executeCommand(command, args) {
    ...
    if (command === 'system.health') {
        try {
            ...
            // 1. Disk-Nutzung
            try {
                const df = await (0, safe_exec_js_1.safeExec)('bash', ['-c', "df -h / | tail -1 | awk '{print $5, $4}'"], { timeout: 5_000 });
                ...
                checks.disk = { status: 'error', message: 'df fehlgeschlagen' };
            }
            // 2. SSL-Zertifikat Ablauf
            try {
                const ssl = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                ...
                checks.ssl = { status: 'warning', message: 'SSL-Check fehlgeschlagen' };
            }
            // 3. OS-Updates ausstehend
            try {
                const updates = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                    "apt list --upgradable 2>/dev/null | grep -c upgradable || echo 0"
                ], { timeout: 15_000 });
                const count = parseInt(updates.stdout.trim()) || 0;
                const security = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                ...
                checks.osUpdates = { status: 'warning', message: 'apt check fehlgeschlagen' };
            }
            // 4. Docker-Container Status
            try {
                const docker = await (0, safe_exec_js_1.safeExec)('docker', ['ps', '--format', '{{.Names}}|{{.Status}}'], { timeout: 10_000 });
                ...
                checks.docker = { status: 'warning', message: 'Docker check fehlgeschlagen' };
            }
            // 5. Synapse-Version
            try {
                const ver = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
            ...
            try {
                const backup = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                ...
                checks.backup = { status: 'warning', message: 'Backup-Check fehlgeschlagen' };
            }
            ...
            try {
                ...
                try {
                    const wcHash = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                    ...
                    if (hash)
                        parts.push(`Web-Client: ${hash}`);
                }
                catch { /* ignore */ }
                // Agent-Version aus package.json
                try {
                    const agentVer = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                        "cat /opt/prilog-agent/package.json /opt/prilog/agent/package.json 2>/dev/null | grep '\"version\"' | head -1 | grep -oP '\\d+\\.\\d+\\.\\d+'"
                    ], { timeout: 5_000 });
                    if (agentVer.stdout.trim())
                        parts.push(`Agent: v${agentVer.stdout.trim()}`);
                }
                catch { /* ignore */ }
                // Agent Git-Commit
                try {
                    const agentGit = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                        "cd /opt/prilog-agent 2>/dev/null || cd /opt/prilog/agent 2>/dev/null; git rev-parse --short HEAD 2>/dev/null"
                    ], { timeout: 5_000 });
                    if (agentGit.stdout.trim())
                        parts.push(`Commit: ${agentGit.stdout.trim()}`);
                }
                catch { /* ignore */ }
                ...
                checks.prilogVersion = { status: 'warning', message: 'Version-Check fehlgeschlagen' };
            }
            // 8. IP-Adressen (oeffentlich + Tailscale)
            try {
                const parts = [];
                try {
                    const pub = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                        "curl -s --max-time 5 https://ifconfig.me 2>/dev/null || curl -s --max-time 5 https://api.ipify.org 2>/dev/null || echo '?'"
                    ], { timeout: 10_000 });
                    if (pub.stdout.trim())
                        parts.push(`Public: ${pub.stdout.trim()}`);
                }
                catch { /* ignore */ }
                try {
                    const ts = await (0, safe_exec_js_1.safeExec)('bash', ['-c',
                        "tailscale ip -4 2>/dev/null || echo '?'"
                    ], { timeout: 5_000 });
                    if (ts.stdout.trim() && ts.stdout.trim() !== '?')
                        parts.push(`Tailscale: ${ts.stdout.trim()}`);
                }
                catch { /* ignore */ }
                checks.network = {
                    status: 'ok',
                ...
                checks.network = { status: 'warning', message: 'Netzwerk-Check fehlgeschlagen' };
            }
    ...
    if (command === 'managed_rooms.status') {
        try {
            ...
            for (const candidate of candidates) {
                try {
                    raw = await (0, promises_1.readFile)(candidate, 'utf8');
                    file = candidate;
                    break;
                }
                catch {
            ...
            const entry = (doc?.modules ?? []).find((m) => m?.module === 'synapse_managed_rooms.ManagedRoomsModule');
            return {
                success: true,
